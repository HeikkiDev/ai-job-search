...
1. Check that Notion MCP tools are available in this session (tool names vary by runtime - `mcp__notion__*` in Claude Code, `notion-*` in Copilot CLI for a server registered as `notion`; match on the Notion capability, not an exact prefix). Determine this from the session's own tool list **only** - never by running shell commands like `claude mcp list` or `copilot mcp`, which would interrupt the user with a permission prompt before the graceful exit. If the tools are not available, stop and tell the user how to connect:
   > Notion MCP isn't connected. In Claude Code, run `claude mcp add --transport http notion https://mcp.notion.com/mcp`; in Copilot CLI, add the same HTTP server via `/mcp`. Then start a **new session** (servers added mid-session are only picked up on restart), run `/mcp` there to complete the OAuth login, and re-run `/notion-sync`.
